...
export function currencyMask(
	...
	const numero = parseFloat(value as string)
...
export const dateMask = (dateString: string) => {
	const cleanedDateString = dateString.replace(/\D/g, '')
	const formattedValue = cleanedDateString
		.slice(0, 8)
		.replace(/^(\d{2})(\d)/g, '$1/$2')
...
export function numbersMaskWithLimit(v: string, limit: number): string {
	if (!v) return ''
	v = v.replace(/\D/g, '')
	v = v.slice(0, limit)
	return v
